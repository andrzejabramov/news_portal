...
При регистрации нового пользоваеля ему уходит приветственное письмо на указанным им при регистарации email:   
...
При подписке авторизованного пользователя на категорию публикации из страницы этой публикации
...
в случае, если любым автором создана пубикация по данной категории отпавляется подписанту уведомление на email:
...
В текущей реализации для тестов интервал рассылки дайджеста установлен в 1 мин:
```commandline
scheduler.add_job(
            send_weekly_digest,
...
```
Не забываем запустить в новом окне терминала:
```commandline
python3 manage.py runapscheduler
```
В продакшн переходим на cron.  
