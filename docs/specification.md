...
|---|----------|---------------|---------------------|----------|
| 1 | Открыть список новостей | `http://127.0.0.1:8000/news/` | Отображаются новости (макс. 10 на странице) | [📸](#) |
| 2 | Проверить пагинацию | Клик на `2` в пагинации | Страница 2, в URL `?page=2` | [📸](#) |
| 3 | Перейти на последнюю | Клик на `Последняя »` | Последняя страница с новостями | [📸](#) |
| 4 | Вернуться на первую | Клик на `« Первая` | Страница 1, `?page=1` | [📸](#) |
...
| № | Проверка | Команда / URL | Ожидаемый результат | Скриншот |
|---|----------|---------------|---------------------|----------|
| 1 | Открыть поиск | `http://127.0.0.1:8000/news/search/` | Форма с 3 полями (Название, Автор, Дата) | [📸](#) |
| 2 | Поиск по названию | Ввести `Тест`, нажать Найти | Фильтрация по заголовку | [📸](#) |
| 3 | Поиск по автору | Ввести `admin`, нажать Найти | Фильтрация по автору | [📸](#) |
| 4 | Поиск по дате | Выбрать `19.02.2026`, нажать Найти | Новости ≥ этой даты | [📸](#) |
| 5 | Комбинация фильтров | `Тест` + `admin` + дата | Все фильтры работают вместе | [📸](#) |
| 6 | Пагинация с фильтрами | Клик на страницу 2 | Фильтры сохраняются в URL | [📸](#) |
| 7 | Сброс фильтров | Клик на `Сбросить` | Все новости без фильтров | [📸](#) |
...
| № | Проверка | Команда / URL | Ожидаемый результат | Скриншот |
|---|----------|---------------|---------------------|----------|
| 1 | Форма создания новости | `http://127.0.0.1:8000/news/news/create/` | Форма (Заголовок, Текст, Категории) | [📸](#) |
| 2 | Создать новость | Заполнить форму, Сохранить | Редирект на `/news/<pk>/` | [📸](#) |
| 3 | Проверить тип | Открыть `/news/<pk>/` | Тип = `Новость` | [📸](#) |
| 4 | Форма создания статьи | `http://127.0.0.1:8000/news/articles/create/` | Та же форма | [📸](#) |
| 5 | Создать статью | Заполнить форму, Сохранить | Тип = `Статья` | [📸](#) |
| 6 | Статья в списке | Открыть `/news/` | Статья НЕ отображается (только новости) | [📸](#) |
...
| № | Проверка | Команда / URL | Ожидаемый результат | Скриншот |
|---|----------|---------------|---------------------|----------|
| 1 | Открыть редактирование | `http://127.0.0.1:8000/news/news/<pk>/edit/` | Форма с заполненными данными | [📸](#) |
| 2 | Изменить заголовок | Ввести новый, Сохранить | Заголовок обновлён | [📸](#) |
| 3 | Редактирование статьи | `http://127.0.0.1:8000/news/articles/<pk>/edit/` | Форма для статьи | [📸](#) |
...
| № | Проверка | Команда / URL | Ожидаемый результат | Скриншот |
|---|----------|---------------|---------------------|----------|
| 1 | Открыть удаление | `http://127.0.0.1:8000/news/news/<pk>/delete/` | Страница подтверждения | [📸](#) |
| 2 | Подтвердить удаление | Клик `Да, удалить` | Редирект на `/news/`, пост удалён | [📸](#) |
| 3 | Проверка в БД | `python manage.py shell` → `Post.objects.count()` | Количество уменьшилось на 1 | [📸](#) |
